// compile using gcc: 
// gcc -Ofast whets.c -o whets -lm -DDP -DGTODay -DUNIXBENCH -static
// compile using clang:
// clang -Ofast -march=native whets.c -o whets-clang -rpath /mnt/DataDisk2/dad/llvm/install/lib -static -lm -DDP -DGTODay -DUNIXBENCH
...
char	*argv[];
{
 ...
 printf ("Roy_Longbottom@compuserve.com\n\n");
	...
	SPDP x,y,z;
	SPDP t0;
	SPDP timea,timeb, dtime();
	SPDP esave[4];

	t0 = t;
	Check=0.0;
printf(" xtra, x100, calibrate = %li,%li,%i\n",xtra, x100, calibrate);
	n1 = 12*x100;
	n2 = 14*x100;
	n3 = 345*x100;
	n4 = 210*x100;
...
SPDP dtime()
{
 ...
 unsigned long time_in_micros = 1000000 * tnow.tv_sec + tnow.tv_usec;
 // q = (SPDP)tnow.tv_sec + (SPDP)tnow.tv_usec * 1.0e-6;
 q = (SPDP)time_in_micros * 1.0e-6;
